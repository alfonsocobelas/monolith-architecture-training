...
export class Cursor {
  ...
  private static generateFiltersHash(filters: Filters): string {
    const deterministicFilters = [...filters.filters].sort((a, b) => a.field.value.localeCompare(b.field.value))

    const serializedFilters = JSON.stringify(deterministicFilters)

    return crypto.createHash('sha256').update(serializedFilters).digest(Encoding.BASE64)
  }

  private static generateOrdersHash(orders: Orders): string {
    const sortedOrders = [...orders.orders].sort((a, b) => a.orderBy.value.localeCompare(b.orderBy.value))
    ...
    return crypto.createHash('sha256').update(serializedOrders).digest(Encoding.BASE64)
  }
}
