    ...
    describe('description validation', () => {
      ...
      it('should fail if normalized description is less than 2 character', () => {
        fc.assert(
          fc.property(
            fc
              .string({ minLength: 0, maxLength: LIMITS.DESCRIPTION.MIN_LENGTH - 1 })
              .filter(s => normalizeString(s).length > 0),
            shortDescription => {
              const builder = IssueBuilder.anIssue().withDescription(shortDescription)

              expect(() => builder.create()).toThrow(
                `Description must be at least ${LIMITS.DESCRIPTION.MIN_LENGTH} characters`
              )
            }
          )
        )
      ...
      it('should fail if  normalized description exceeds 500 characters', () => {
        fc.assert(
          fc.property(
            fc
              .string({ minLength: LIMITS.DESCRIPTION.MAX_LENGTH + 1 })
            ...
            longDescription => {
              ...
              expect(() => builder.create()).toThrow(
                `Description must be less than or equal to ${LIMITS.DESCRIPTION.MAX_LENGTH} characters`
              )
            }
          )
        )
