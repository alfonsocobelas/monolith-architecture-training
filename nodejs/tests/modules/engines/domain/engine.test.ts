    ...
    describe('serial number validation', () => {
      ...
      it('should fail if serial number is less than 1 characters', () => {
        fc.assert(
          fc.property(
            fc
              .string({ minLength: 0, maxLength: LIMITS.SERIAL_NUMBER.MIN_LENGTH - 1 })
            ...
            shortSn => {
              ...
              expect(() => builder.create()).toThrow(
                `Serial number must be at least ${LIMITS.SERIAL_NUMBER.MIN_LENGTH} characters`
              )
            }
          )
        )
      ...
      it('should fail if serial number is greater than 50 characters', () => {
        fc.assert(
          fc.property(
            fc
              .string({ minLength: LIMITS.SERIAL_NUMBER.MAX_LENGTH + 1 })
            ...
            longSn => {
              ...
              expect(() => builder.create()).toThrow(
                `Serial number must be less than or equal to ${LIMITS.SERIAL_NUMBER.MAX_LENGTH} characters`
              )
            }
          )
        )
  ...
  describe('Business rules', () => {
    ...
    describe('on installation', () => {
      ...
      it('should fail to install if already installed', () => {
        ...
        expect(() => engine.installInAircraft(uuidv7())).toThrow(
          `Engine ${engine.id} is already installed on the other aircraft`
        )
      })
      ...
      it('should fail to install if status is not OPERATIONAL', () => {
        ...
        expect(() => engine.installInAircraft(uuidv7())).toThrow('Only operational engines can be installed')
      })

      it('should not allow installing an engine with health score below 60', () => {
        fc.assert(
          fc.property(fc.double({ min: 0, max: LIMITS.HEALTH_SCORE.MIN - 0.1, noNaN: true }), lowHealthScore => {
            const engine = EngineBuilder.anEngine()
            ...
            expect(() => engine.installInAircraft(uuidv7())).toThrow(
              `Engine health score must be at least ${LIMITS.HEALTH_SCORE.MIN} to be installed`
            )
          })
        )
      })
