  ...
  describe('Invariants', () => {
    ...
    describe('name validation', () => {
      ...
      it('should fail if name is less than 2 characters', () => {
        fc.assert(
          fc.property(
            fc.string({ maxLength: LIMIT.NAME.MIN_LENGTH - 1 }).filter(str => normalizeString(str).length > 0),
            shortName => {
      ...
      it('should fail if name is greater than 100 characters', () => {
        fc.assert(
          fc.property(fc.string({ minLength: LIMIT.NAME.MAX_LENGTH + 1 }), longName => {
            const builder = CompanyBuilder.aCompany().withName(longName)

            expect(() => builder.create()).toThrow(
              `Name must be less than or equal to ${LIMIT.NAME.MAX_LENGTH} characters`
            )
          })
        )
      })
