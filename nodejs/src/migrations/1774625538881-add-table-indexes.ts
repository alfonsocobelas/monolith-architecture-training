...
export class AddTableIndexes1774625538881 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TYPE "public"."fleets_type_enum" AS ENUM('CARGO', 'PASSENGER', 'MILITARY', 'PRIVATE', 'SPECIALIZED')`);
        await queryRunner.query(`CREATE TYPE "public"."fleets_status_enum" AS ENUM('DRAFT', 'OPERATIVE', 'RETIRED', 'SCRAPPED')`);
        await queryRunner.query(`CREATE TABLE "fleets" ("id" uuid NOT NULL, "aircraftIds" uuid array NOT NULL, "companyId" uuid NOT NULL, "name" character varying(100) NOT NULL, "operationRegion" "public"."fleets_operationregion_enum" NOT NULL, "type" "public"."fleets_type_enum" NOT NULL, "maintenanceBudget" double precision NOT NULL, "status" "public"."fleets_status_enum" NOT NULL, "createdAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deletedAt" TIMESTAMP WITH TIME ZONE, CONSTRAINT "UQ_fdcdafb112075c2916d766f0de8" UNIQUE ("name"), CONSTRAINT "CHK_3766a650399b57d4005c9fae07" CHECK (char_length("name") >= 2 AND char_length("name") <= 100), CONSTRAINT "PK_18a71e919faac62c1da6b5f8754" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE UNIQUE INDEX "IDX_FLEET_NAME" ON "fleets" ("name") `);
        await queryRunner.query(`CREATE TYPE "public"."aircraft_models_status_enum" AS ENUM('DRAFT', 'WITHDRAW', 'OPERATIONAL', 'DECOMMISSIONED')`);
        await queryRunner.query(`CREATE TABLE "aircraft_models" ("id" uuid NOT NULL, "name" character varying(30) NOT NULL, "code" character varying(10) NOT NULL, "manufacturer" character varying(20) NOT NULL, "passengerCapacity" integer NOT NULL, "numEngines" integer NOT NULL, "status" "public"."aircraft_models_status_enum" NOT NULL, "createdAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deletedAt" TIMESTAMP WITH TIME ZONE, CONSTRAINT "UQ_89570632115e51ceb3faaddfb1b" UNIQUE ("code"), CONSTRAINT "CHK_258b687e16130010acfc3d83db" CHECK ("numEngines" >= 1 AND "numEngines" <= 6), CONSTRAINT "CHK_1f831a04b6cb810125b7495c9e" CHECK ("passengerCapacity" >= 1 AND "passengerCapacity" <= 400), CONSTRAINT "CHK_b092b595d7d5cac186d0a9d39d" CHECK ("manufacturer" IS NOT NULL AND LENGTH("manufacturer") >= 2 AND LENGTH("manufacturer") <= 20), CONSTRAINT "CHK_7b7c1807ce8590d6a4448cca8a" CHECK ("code" IS NOT NULL AND LENGTH("code") >= 2 AND LENGTH("code") <= 10), CONSTRAINT "CHK_1db376fc06a3ead6f72bd36ff8" CHECK ("name" IS NOT NULL AND LENGTH("name") >= 2 AND LENGTH("name") <= 30), CONSTRAINT "PK_53c80d0e1bf02630aec8beb644d" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE UNIQUE INDEX "IDX_AIRCRAFT_MODEL_CODE" ON "aircraft_models" ("code") `);
        await queryRunner.query(`CREATE TYPE "public"."aircrafts_status_enum" AS ENUM('DRAFT', 'ACTIVE', 'MAINTENANCE', 'RETIRED', 'STORED', 'WRITTEN_OFF', 'SCRAPPED')`);
        ...
        await queryRunner.query(`CREATE TYPE "public"."issues_partcategory_enum" AS ENUM('ENGINE', 'FUSELAGE', 'AVIONICS')`);
        await queryRunner.query(`CREATE TABLE "issues" ("id" uuid NOT NULL, "code" character varying(20) NOT NULL, "aircraftId" uuid, "engineId" uuid, "description" character varying(500) NOT NULL, "severity" "public"."issues_severity_enum" NOT NULL, "requiresGrounding" boolean NOT NULL DEFAULT false, "partCategory" "public"."issues_partcategory_enum" NOT NULL, "createdAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deletedAt" TIMESTAMP WITH TIME ZONE, CONSTRAINT "UQ_5be3a6bb806b982704cb1adffd6" UNIQUE ("code"), CONSTRAINT "CHK_eb3d9d63ffd35e0d63a3d35e66" CHECK (char_length("description") >= 2 AND char_length("description") <= 500), CONSTRAINT "CHK_0599ce5bc5182d169b274eb13d" CHECK (char_length("code") >= 2 AND char_length("code") <= 20), CONSTRAINT "PK_9d8ecbbeff46229c700f0449257" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE UNIQUE INDEX "IDX_ISSUE_CODE" ON "issues" ("code") `);
        await queryRunner.query(`ALTER TABLE "engines" ADD CONSTRAINT "FK_4bff46830e20a5d185b074c3742" FOREIGN KEY ("aircraftId") REFERENCES "aircrafts"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "fleets" ADD CONSTRAINT "FK_ccd66320317fcdded55b1e73b56" FOREIGN KEY ("companyId") REFERENCES "companies"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "aircrafts" ADD CONSTRAINT "FK_ec1bd4474866988540f4bc7fdb0" FOREIGN KEY ("modelId") REFERENCES "aircraft_models"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "aircrafts" ADD CONSTRAINT "FK_1e4e2025dd5256544f37117bea2" FOREIGN KEY ("fleetId") REFERENCES "fleets"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "issues" ADD CONSTRAINT "FK_bea8e1814ec8020f21a646497a8" FOREIGN KEY ("aircraftId") REFERENCES "aircrafts"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "issues" ADD CONSTRAINT "FK_303ca0887939207bb454ea70c6a" FOREIGN KEY ("engineId") REFERENCES "engines"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
