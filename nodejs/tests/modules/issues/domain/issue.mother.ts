...
export class IssueMother {
  static fromInput(input: Partial<IssuePrimitiveProps>): Issue {
    return IssueBuilder.anIssue()
      .withProps(input as Partial<IssueProps>)
      .build()
  }
  ...
  static avionics(aircraftId: string = uuidv7()) {
    return IssueBuilder.anIssue().withPartCategory(IssuePartCategory.AVIONICS).withAircraftId(aircraftId).build()
  }

  static engine(engineId: string = uuidv7()) {
    return IssueBuilder.anIssue().withPartCategory(IssuePartCategory.ENGINE).withEngineId(engineId).build()
  }
}
